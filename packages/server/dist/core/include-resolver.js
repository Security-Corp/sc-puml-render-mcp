 ...
 */
export async function resolveIncludes(source, opts, request = {}) {
...
function normalizeOptions(options) {
    ...
    if (!Number.isInteger(options.maxTotalBytes) || options.maxTotalBytes <= 0) {
        throw new IncludeResolutionError("MAX_TOTAL_SIZE_EXCEEDED", "maxTotalBytes must be a positive integer");
    }
    return {
...
async function resolveLoadedSource(content, state, depth, stack) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (depth + 1 > state.options.maxDepth) {
            throw new IncludeResolutionError("MAX_DEPTH_EXCEEDED", `Maximum include depth exceeded at ${content.uri}:${directive.lineNumber}`, { maxDepth: state.options.maxDepth, uri: content.uri, line: directive.lineNumber });
        }
        ...
        if (knownRemoteUri &&
            ...
            state.includedUris.has(knownRemoteUri)) {
            throw new IncludeResolutionError("INCLUDE_ONCE_REPEATED", `!include_once target was included more than once: ${knownRemoteUri}`, { uri: knownRemoteUri });
        }
        ...
        if (directive.command === "include_once" && state.includedUris.has(included.uri)) {
            throw new IncludeResolutionError("INCLUDE_ONCE_REPEATED", `!include_once target was included more than once: ${included.uri}`, { uri: included.uri });
        }
...
function addSize(content, state) {
    state.totalBytes += Buffer.byteLength(content.text, "utf8");
    if (state.totalBytes > state.options.maxTotalBytes) {
        throw new IncludeResolutionError("MAX_TOTAL_SIZE_EXCEEDED", `Resolved PlantUML include graph exceeds ${state.options.maxTotalBytes} bytes`, { maxTotalBytes: state.options.maxTotalBytes, totalBytes: state.totalBytes });
    }
}
function parseIncludeDirective(line, lineNumber) {
    ...
    if (!isIncludeCommand(command) || rawTarget === undefined) {
        if (command === "includesub") {
            throw new IncludeResolutionError("UNSUPPORTED_INCLUDE_SELECTOR", "!includesub is not supported by the Faz 2 include resolver");
        }
        return undefined;
    }
...
function isIncludeCommand(value) {
    return (value === "include" ||
        value === "include_many" ||
        value === "include_once" ||
        value === "includeurl");
}
function isIncludeOnceByDefault(command) {
    return command === "include" || command === "includeurl";
}
function stripInlinePlantUmlComment(value) {
    return value.replace(/\s+'.*$/u, "");
}
async function loadIncludedSource(directive, current, state) {
    ...
    if (!current.path) {
        throw new IncludeResolutionError("LOCAL_INCLUDE_UNAVAILABLE", `Cannot resolve local include without a filesystem-backed source at ${current.uri}:${directive.lineNumber}`, { uri: current.uri, line: directive.lineNumber, target });
    }
    return state.source.load({ path: target, includingPath: current.path });
}
function normalizeIncludeTarget(target) {
...
function rejectUnsupportedSelector(target) {
    if (/![A-Za-z0-9_-]+$/u.test(target) && !isHttpUrl(target)) {
        throw new IncludeResolutionError("UNSUPPORTED_INCLUDE_SELECTOR", `PlantUML include block selectors are not supported in Faz 2: ${target}`, { target });
    }
}
async function loadRemoteInclude(target, current, state, directive) {
    const url = parseRemoteUrl(target, current.uri, directive);
    assertRemoteAllowed(url, state, current, directive);
    const response = await fetch(url, { redirect: "manual" });
    if (isRedirect(response.status)) {
        throw new IncludeResolutionError("REMOTE_REDIRECT_NOT_ALLOWED", `Remote include redirects are not followed in Faz 2: ${url.href}`, { url: url.href, location: response.headers.get("location") ?? undefined });
    }
    const finalUrl = response.url ? new URL(response.url) : url;
    assertRemoteAllowed(finalUrl, state, current, directive);
    if (!response.ok) {
        throw new IncludeResolutionError("INVALID_REMOTE_URL", `Remote include failed with HTTP ${response.status}: ${finalUrl.href}`, { url: finalUrl.href, status: response.status });
    }
    const contentLength = response.headers.get("content-length");
    if (contentLength && state.totalBytes + Number(contentLength) > state.options.maxTotalBytes) {
        throw new IncludeResolutionError("MAX_TOTAL_SIZE_EXCEEDED", `Remote include would exceed ${state.options.maxTotalBytes} bytes: ${finalUrl.href}`, { url: finalUrl.href, maxTotalBytes: state.options.maxTotalBytes });
    }
    return {
...
function remoteIncludeUriIfKnown(directive, current) {
    const target = normalizeIncludeTarget(directive.target);
    if (directive.command !== "includeurl" && !isHttpUrl(target)) {
        return undefined;
    }
...
async function readRemoteTextWithLimit(response, state, url) {
    const remainingBytes = state.options.maxTotalBytes - state.totalBytes;
    if (remainingBytes <= 0) {
        throw new IncludeResolutionError("MAX_TOTAL_SIZE_EXCEEDED", `Remote include would exceed ${state.options.maxTotalBytes} bytes: ${url.href}`, { url: url.href, maxTotalBytes: state.options.maxTotalBytes });
    }
    ...
    while (true) {
        ...
        if (bytesRead > remainingBytes) {
            await reader.cancel();
            throw new IncludeResolutionError("MAX_TOTAL_SIZE_EXCEEDED", `Remote include exceeds ${state.options.maxTotalBytes} bytes: ${url.href}`, { url: url.href, maxTotalBytes: state.options.maxTotalBytes });
        }
        chunks.push(value);
    }
...
function parseRemoteUrl(target, currentUri, directive) {
    try {
        if (isHttpUrl(target)) {
    ...
    throw new IncludeResolutionError("INVALID_REMOTE_URL", `!includeurl requires an absolute http(s) URL at ${directive.lineNumber}`, { target, line: directive.lineNumber });
}
...
function assertRemoteAllowed(url, state, current, directive) {
    if (state.remoteAllowlist.size === 0) {
        throw new IncludeResolutionError("REMOTE_INCLUDE_DISABLED", `Remote includes are disabled by default: ${url.href}`, { url: url.href, uri: current.uri, line: directive.lineNumber });
    }
    const host = url.host.toLowerCase();
    const hostname = url.hostname.toLowerCase();
    if (!state.remoteAllowlist.has(host) && !state.remoteAllowlist.has(hostname)) {
        throw new IncludeResolutionError("REMOTE_HOST_NOT_ALLOWED", `Remote include host is not allowlisted: ${url.host}`, { url: url.href, host: url.host, uri: current.uri, line: directive.lineNumber });
    }
}
